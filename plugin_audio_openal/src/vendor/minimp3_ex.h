...
int mp3dec_load_cb(mp3dec_t *dec, mp3dec_io_t *io, uint8_t *buf, size_t buf_size, mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb, void *user_data) {
    ...
    do {
        ...
        const uint8_t *hdr;
        if (io) {
            if (!eof && filled - consumed < MINIMP3_BUF_SIZE) { /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
...
static int mp3dec_load_index(void *user_data, const uint8_t *frame, int frame_size, int free_format_bytes, size_t buf_size, uint64_t offset, mp3dec_frame_info_t *info) {
    ...
    if (!dec->buffer_samples && dec->index.num_frames < 256) { /* for some cutted mp3 frames, bit-reservoir not filled and decoding can't be started from first frames */
...
int mp3dec_ex_seek(mp3dec_ex_t *dec, uint64_t position) {
    ...
    if (!dec->indexes_built) { /* no index created yet (vbr tag used to calculate track length or MP3D_DO_NOT_SCAN open flag used) */
        dec->indexes_built = 1;
        dec->samples = 0;
        dec->buffer_samples = 0;
        if (dec->io) {
    ...
    while ((i + 1) < dec->index.num_frames && !dec->index.frames[i].sample && !dec->index.frames[i + 1].sample) { /* skip not decodable first frames */
        const uint8_t *hdr;
        if (dec->io) {
            hdr = dec->file.buffer;
...
size_t mp3dec_ex_read_frame(mp3dec_ex_t *dec, mp3d_sample_t **buf, mp3dec_frame_info_t *frame_info, size_t max_samples) {
    ...
    *buf = NULL;
    ...
    while (dec->buffer_consumed == dec->buffer_samples) {
        ...
        if (dec->io) {
            if (!eof && (dec->input_filled - dec->input_consumed) < MINIMP3_BUF_SIZE) { /* keep minimum 10 consecutive mp3 frames (~16KB) worst case */
        ...
        } else if (dec->to_skip) { /* In mp3 decoding not always can start decode from any frame because of bit reservoir,

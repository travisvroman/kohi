...
void *vulkan_alloc_reallocation(void *user_data, void *original, size_t size,
  ...
  if (alloc_alignment != alignment) {
    KERROR("Attempted realloc using a different alignment of %llu than the "
           "original of %hu.",
...
b8 vulkan_renderer_backend_initialize(renderer_plugin *plugin,
  ...
  if (!create_vulkan_allocator(context, context->allocator)) {
    ...
    KFATAL("Failed to create custom Vulkan allocator. Continuing using the "
...
// non-release builds.
  ...
  if (!context->pfnSetDebugUtilsObjectNameEXT) {
    KWARN("Unable to load function pointer for vkSetDebugUtilsObjectNameEXT. "
  ...
  if (!context->pfnSetDebugUtilsObjectTagEXT) {
    KWARN("Unable to load function pointer for vkSetDebugUtilsObjectTagEXT. "
          "Debug functions associated with this will not work.");
  }
  ...
  if (!context->pfnCmdBeginDebugUtilsLabelEXT) {
    KWARN("Unable to load function pointer for vkCmdBeginDebugUtilsLabelEXT. "
          "Debug functions associated with this will not work.");
  }
  ...
  if (!context->pfnCmdEndDebugUtilsLabelEXT) {
    KWARN("Unable to load function pointer for vkCmdEndDebugUtilsLabelEXT. "
          "Debug functions associated with this will not work.");
  }
#endif
...
b8 vulkan_renderer_backend_frame_begin(renderer_plugin *plugin,
  ...
  if (context->recreating_swapchain) {
    ...
    if (!vulkan_result_is_success(result)) {
      KERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: "
             "'%s'",
  ...
  if (context->framebuffer_size_generation !=
    ...
    VkResult result = vkDeviceWaitIdle(device->logical_device);
    if (!vulkan_result_is_success(result)) {
      KERROR("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (2) failed: "
             "'%s'",
             vulkan_result_string(result, true));
      return false;
    }
...
void vulkan_renderer_texture_write_data(renderer_plugin *plugin, texture *t,
  ...
  if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_STAGING, size, false,
                                    &staging)) {
...
void vulkan_renderer_texture_read_data(renderer_plugin *plugin, texture *t,
  ...
  // Create a staging buffer and load data into it.
  renderbuffer staging;
  if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, size, false,
                                    &staging)) {
...
void vulkan_renderer_texture_read_pixel(renderer_plugin *plugin, texture *t,
  ...
  // Create a staging buffer and load data into it.
  renderbuffer staging;
  if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_READ, sizeof(u8) * 4,
                                    false, &staging)) {
...
b8 vulkan_renderer_geometry_create(renderer_plugin *plugin, geometry *geometry,
  ...
  if (!vertex_count || !vertices) {
    KERROR("vulkan_renderer_create_geometry requires vertex data, and none was "
  ...
  if (!internal_data) {
    KFATAL("vulkan_renderer_create_geometry failed to find a free index for a "
           "new geometry upload. Adjust config to allow for more.");
    return false;
  }
  ...
  if (!renderer_renderbuffer_allocate(&context->object_vertex_buffer,
                                      total_size,
                                      &internal_data->vertex_buffer_offset)) {
    KERROR("vulkan_renderer_create_geometry failed to allocate from the vertex "
           "buffer!");
    return false;
  }

  // Load the data.
  if (!renderer_renderbuffer_load_range(&context->object_vertex_buffer,
                                        internal_data->vertex_buffer_offset,
                                        total_size, vertices)) {
    KERROR("vulkan_renderer_create_geometry failed to upload to the vertex "
           "buffer!");
    return false;
  }
  ...
  if (index_count && indices) {
    ...
    if (!renderer_renderbuffer_allocate(&context->object_index_buffer,
                                        total_size,
                                        &internal_data->index_buffer_offset)) {
      KERROR("vulkan_renderer_create_geometry failed to allocate from the "
             "index buffer!");
      return false;
    }

    if (!renderer_renderbuffer_load_range(&context->object_index_buffer,
                                          internal_data->index_buffer_offset,
                                          total_size, indices)) {
      KERROR("vulkan_renderer_create_geometry failed to upload to the index "
             "buffer!");
      return false;
    }
  }
  ...
  if (is_reupload) {
    // Free vertex data
    if (!renderer_renderbuffer_free(&context->object_vertex_buffer,
                                    old_range.vertex_element_size *
                                        old_range.vertex_count,
                                    old_range.vertex_buffer_offset)) {
      KERROR("vulkan_renderer_create_geometry free operation failed during "
             "reupload of vertex data.");
      return false;
    }

    // Free index data, if applicable
    if (old_range.index_element_size > 0) {
      if (!renderer_renderbuffer_free(&context->object_index_buffer,
                                      old_range.index_element_size *
                                          old_range.index_count,
                                      old_range.index_buffer_offset)) {
        KERROR("vulkan_renderer_create_geometry free operation failed during "
               "reupload of index data.");
        return false;
      }
    }
  }
...
void vulkan_renderer_geometry_destroy(renderer_plugin *plugin,
                                      geometry *geometry) {
  vulkan_context *context = (vulkan_context *)plugin->internal_context;
  if (geometry && geometry->internal_id != INVALID_ID) {
    ...
    if (!renderer_renderbuffer_free(&context->object_vertex_buffer,
                                    internal_data->vertex_element_size *
                                        internal_data->vertex_count,
                                    internal_data->vertex_buffer_offset)) {
      KERROR("vulkan_renderer_destroy_geometry failed to free vertex buffer "
             "range.");
    }
    ...
    if (internal_data->index_element_size > 0) {
      if (!renderer_renderbuffer_free(&context->object_index_buffer,
                                      internal_data->index_element_size *
                                          internal_data->index_count,
                                      internal_data->index_buffer_offset)) {
        KERROR("vulkan_renderer_destroy_geometry failed to free index buffer "
               "range.");
      }
    }
...
void vulkan_renderer_geometry_draw(renderer_plugin *plugin,
...
b8 vulkan_renderer_shader_create(renderer_plugin *plugin, shader *s,
  ...
  for (u8 i = 0; i < stage_count; ++i) {
    ...
    case SHADER_STAGE_GEOMETRY:
      KWARN("vulkan_renderer_shader_create: VK_SHADER_STAGE_GEOMETRY_BIT is "
            "set but not yet supported.");
      vk_stages[i] = VK_SHADER_STAGE_GEOMETRY_BIT;
      break;
    case SHADER_STAGE_COMPUTE:
      KWARN("vulkan_renderer_shader_create: SHADER_STAGE_COMPUTE is set but "
            "not yet supported.");
  ...
  for (u32 i = 0; i < stage_count; i++) {
    ...
    default:
      // Go to the next type.
      KERROR("vulkan_shader_create: Unsupported shader stage flagged: %d. "
             "Stage ignored.",
             stages[i]);
      continue;
    }
...
b8 vulkan_renderer_shader_initialize(renderer_plugin *plugin, shader *s) {
  ...
  for (u32 i = 0; i < internal_shader->config.stage_count; ++i) {
    if (!create_shader_module(context, internal_shader,
      ...
      KERROR("Unable to create %s shader module for '%s'. Shader will be "
             "destroyed.",
  ...
  pipeline_config.attributes =
      internal_shader->config.attributes; // shader->attributes,
  pipeline_config.descriptor_set_layout_count =
      internal_shader->config.descriptor_set_count;
  pipeline_config.descriptor_set_layouts =
      internal_shader->descriptor_set_layouts;
  ...
  // buffer resizing.
  u64 total_buffer_size =
      s->global_ubo_stride +
      (s->ubo_stride * VULKAN_MAX_MATERIAL_COUNT); // global + (locals)
  if (!renderer_renderbuffer_create(RENDERBUFFER_TYPE_UNIFORM,
                                    total_buffer_size, true,
                                    &internal_shader->uniform_buffer)) {
  ...
  if (!renderer_renderbuffer_allocate(&internal_shader->uniform_buffer,
                                      s->global_ubo_stride,
                                      &s->global_ubo_offset)) {
...
static VkSamplerAddressMode convert_repeat_type(const char *axis,
  ...
  default:
    KWARN("convert_repeat_type(axis='%s') Type '%x' not supported, defaulting "
          "to repeat.",
          axis, repeat);
    return VK_SAMPLER_ADDRESS_MODE_REPEAT;
  }
...
static VkFilter convert_filter_type(const char *op, texture_filter filter) {
  ...
  default:
    KWARN("convert_filter_type(op='%s'): Unsupported filter type '%x', "
  ...
  *out_instance_id = INVALID_ID;
  ...
  if (size > 0) {
    if (!renderer_renderbuffer_allocate(&internal->uniform_buffer, size,
      ...
      KERROR("vulkan_material_shader_acquire_resources failed to acquire ubo "
             "space");
      return false;
    }
  }
...
b8 vulkan_renderer_shader_instance_resources_release(renderer_plugin *plugin,
  ...
  if (s->ubo_stride != 0) {
    if (!renderer_renderbuffer_free(&internal->uniform_buffer, s->ubo_stride,
                                    instance_state->offset)) {
      KERROR("vulkan_renderer_shader_release_instance_resources failed to free "
    ...
    if (attachment_config->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOUR) {
      ...
      if (attachment_config->load_operation ==
        ...
        if (attachment_config->load_operation ==
          ...
          if (do_clear_colour) {
            KWARN("Colour attachment load operation set to load, but is also "
          ...
          KFATAL("Invalid and unsupported combination of load operation (0x%x) "
      ...
      if (attachment_config->store_operation ==
        ...
        KFATAL("Invalid store operation (0x%x) set for depth attachment. Check "
               "configuration.",
      ...
      if (attachment_config->load_operation ==
        ...
        if (attachment_config->load_operation ==
            RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD) {
          if (do_clear_depth) {
            KWARN("Depth attachment load operation set to load, but is also "
                  "set to clear. This combination is invalid, and will err "
                  "toward clearing. Verify attachment configuration.");
            attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
          } else {
...
texture *vulkan_renderer_window_attachment_get(renderer_plugin *plugin,
  ...
  if (index >= context->swapchain.image_count) {
    KFATAL("Attempting to get colour attachment index out of range: %d. "
           "Attachment count: %d",
...
texture *vulkan_renderer_depth_attachment_get(renderer_plugin *plugin,
  ...
  vulkan_context *context = (vulkan_context *)plugin->internal_context;
  if (index >= context->swapchain.image_count) {
    KFATAL("Attempting to get depth attachment index out of range: %d. "
           "Attachment count: %d",
           index, context->swapchain.image_count);
    return 0;
  }
...
b8 vulkan_buffer_create_internal(renderer_plugin *plugin,
                                 renderbuffer *buffer) {
  vulkan_context *context = (vulkan_context *)plugin->internal_context;
  if (!buffer) {
    KERROR(
        "vulkan_buffer_create_internal requires a valid pointer to a buffer.");
    return false;
  }
...
void vulkan_buffer_destroy_internal(renderer_plugin *plugin,
                                    renderbuffer *buffer) {
  vulkan_context *context = (vulkan_context *)plugin->internal_context;
  if (buffer) {
...
b8 vulkan_buffer_read(renderer_plugin *plugin, renderbuffer *buffer, u64 offset,
  ...
  if (!buffer || !buffer->internal_data || !out_memory) {
    KERROR("vulkan_buffer_read requires a valid pointer to a buffer and "
...
b8 vulkan_buffer_load_range(renderer_plugin *plugin, renderbuffer *buffer,
  ...
  if (!buffer || !buffer->internal_data || !size || !data) {
    KERROR("vulkan_buffer_load_range requires a valid pointer to a buffer, a "
...
b8 vulkan_buffer_copy_range(renderer_plugin *plugin, renderbuffer *source,
  ...
  if (!source || !source->internal_data || !dest || !dest->internal_data ||
      !size) {
    KERROR("vulkan_buffer_copy_range requires a valid pointers to source and "

            ...
            if (needs_realloc) {
                if (!renderer_renderbuffer_allocate(vertex_buffer, new_vertex_size, &typed_data->pending_data.vertex_buffer_offset)) {
                    KERROR(
                        ...
                        new_vertex_size,
                        typed_data->pending_data.vertex_buffer_offset);
                    return;
                }
                ...
                if (!renderer_renderbuffer_allocate(index_buffer, new_index_size, &typed_data->pending_data.index_buffer_offset)) {
                    KERROR(
                        ...
                        new_index_size,
                        typed_data->pending_data.index_buffer_offset);
                    return;
                }
...
static void reset_pending_data(sui_label_pending_data* data) {
    ...
    if (data->vertex_buffer_data) {
        KASSERT_MSG(data->vertex_buffer_size, "Attempted to free pending vertex data with a size of 0. Pointer to data exists but size is invalid.");
        KASSERT_MSG(data->vertex_buffer_size != INVALID_ID_U64, "Attempted to free pending vertex data with a size of INVALID_ID_U64. Pointer to data exists but size is invalid.");

        kfree(data->vertex_buffer_data, data->vertex_buffer_size, MEMORY_TAG_ARRAY);
        data->vertex_buffer_data = 0;
    }
    if (data->index_buffer_data) {
        KASSERT_MSG(data->index_buffer_size, "Attempted to free pending index data with a size of 0. Pointer to data exists but size is invalid.");
        KASSERT_MSG(data->index_buffer_size != INVALID_ID_U64, "Attempted to free pending index data with a size of INVALID_ID_U64. Pointer to data exists but size is invalid.");

        kfree(data->index_buffer_data, data->index_buffer_size, MEMORY_TAG_ARRAY);

...
b8 render_view_skybox_on_packet_build(const struct render_view* self, struct linear_allocator* frame_allocator, void* data, struct render_view_packet* out_packet) {
    if (!self || !data || !out_packet) {
        KWARN("render_view_skybox_on_packet_build requires valid pointer to view, packet, and data.");
        return false;
    }
...
b8 render_view_skybox_on_render(const struct render_view* self, const struct render_view_packet* packet, const struct frame_data* p_frame_data) {
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p) {
        ...
        if (skybox_data && skybox_data->sb) {
            ...
            b8 needs_update = skybox_data->sb->render_frame_number != p_frame_data->renderer_frame_number;
            shader_system_apply_instance(needs_update);

            // Sync the frame number.
            skybox_data->sb->render_frame_number = p_frame_data->renderer_frame_number;

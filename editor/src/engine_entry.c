...
b8 load_game_lib(application* app) {
    ...
    if (!platform_dynamic_library_load("editor_lib_loaded", &app->game_library)) {
        return false;
    }
...
b8 watched_file_updated(u16 code, void* sender, void* listener_inst, event_context context) {
    if (code == EVENT_CODE_WATCHED_FILE_WRITTEN) {
        ...
        if (!platform_dynamic_library_unload(&app->game_library)) {
            KERROR("Failed to unload editor library");
            return false;
        }
        ...
        string_format(source_file, "%seditor_lib%s", prefix, extension);
        string_format(target_file, "%seditor_lib_loaded%s", prefix, extension);
...
b8 initialize_application(application* app) {
    ...
    string_format(path, "%s%s%s", prefix, "editor_lib", extension);

    if (!platform_watch_file(path, &app->game_library.watch_id)) {
        KERROR("Failed to watch the editor library!");
        return false;
    }

...
b8 resource_system_initialize(u64 *memory_requirement, void *state,
  ...
  if (typed_config->max_loader_count == 0) {
    KFATAL("resource_system_initialize failed because "
    ...
    for (u32 i = 0; i < count; ++i) {
      ...
      if (l->id != INVALID_ID) {
        if (l->type == loader.type) {
          KERROR("resource_system_register_loader - Loader of type %d already "
          ...
          KERROR("resource_system_register_loader - Loader of custom type %s "
...
const char *resource_system_base_path(void) {
  ...
  KERROR("resource_system_base_path called before initialization, returning "
         "empty string.");
  return "";
}

...
static b8 import_obj_file(file_handle *obj_file, const char *out_ksm_filename,
  ...
  for (u64 i = 0; i < count; ++i) {
    ...
    KDEBUG("Geometry de-duplication process starting on geometry object named "
           "'%s'...",
           g->name);
  ...
  while (true) {
    ...
    case 'm': {
      ...
      if (strings_nequali(substr, "map_Kd", 6)) {
        ...
        string_filename_no_extension_from_path(current_config.diffuse_map_name,
                                               texture_file_name);
      } else if (strings_nequali(substr, "map_Ks", 6)) {
        // Is a specular texture map
        string_filename_no_extension_from_path(current_config.specular_map_name,
                                               texture_file_name);
      } else if (strings_nequali(substr, "map_bump", 8)) {
        // Is a bump texture map
        string_filename_no_extension_from_path(current_config.normal_map_name,
                                               texture_file_name);
      }
    } break;
    case 'b': {
...
static b8 write_kmt_file(const char *mtl_file_path, material_config *config) {
  ...
  filesystem_write_line(&f, "version=1"); // TODO: hardcoded version.

...
b8 kasset_importer_bitmap_font_fnt(const struct kasset_importer* self, u64 data_size, const void* data, void* params, struct kasset* out_asset) {
    ...
    // Handle FNT file import.
    {
        ...
        KCOPY_TYPE_CARRAY(typed_asset->pages.data, fnt_asset.pages, kasset_bitmap_font_page, fnt_asset.page_count);

        typed_asset->glyphs = array_kasset_bitmap_font_glyph_create(fnt_asset.glyph_count);
        KCOPY_TYPE_CARRAY(typed_asset->glyphs.data, fnt_asset.glyphs, kasset_bitmap_font_glyph, fnt_asset.glyph_count);

        if (fnt_asset.kerning_count) {
            typed_asset->kernings = array_kasset_bitmap_font_kerning_create(fnt_asset.kerning_count);
            KCOPY_TYPE_CARRAY(typed_asset->kernings.data, fnt_asset.kernings, kasset_bitmap_font_kerning, fnt_asset.kerning_count);
        }
    ...
    // Serialize data and write out kbf file (binary Kohi Bitmap Font).
    {
        ...
        void* serialized_data = kasset_bitmap_font_serialize(out_asset, &serialized_size);

...
typedef struct shadow_map_pass_extended_data {
    ...
    // Per-cascade data.
    shadow_cascade_data cascades[MAX_CASCADE_COUNT];
...
KAPI b8 shadow_rendergraph_node_static_geometries_set(struct rendergraph_node* self, u32 geometry_count, const struct geometry_render_data* geometries);
KAPI b8 shadow_rendergraph_node_terrain_geometries_set(struct rendergraph_node* self, u32 geometry_count, const struct geometry_render_data* geometries);

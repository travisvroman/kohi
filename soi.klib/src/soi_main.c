...
static b8 game_register_events(application* app);
...
static b8 game_register_commands(application* app);
static void game_unregister_commands(application* app);
static void game_command_exit(console_command_context context);
static void game_command_load_zone(console_command_context context);
static void game_command_unload_zone(console_command_context context);
static void game_command_go_to_zone(console_command_context context);
static void game_command_save_zone(console_command_context context);
...
b8 application_boot(struct application* app) {
    ...
    input_keymap_push(&game->global_keymap);

    // Register game events.
    if (!game_register_events(app)) {
        KERROR("%s - Failed to register game events. See logs for details.");
        return false;
    }

    // Register console commands.
    if (!game_register_commands(app)) {
        KERROR("%s - Failed to register game console commands. See logs for details.");
        return false;
    }

    // Set default game mode and keymap
    game->mode = GAME_MODE_WORLD;
    game->current_camera = game->vehicle_camera;
    input_keymap_push(&game->world_keymap);
...
b8 application_initialize(struct application* app) {
    ...
    // Zone system
    {
        ...
        if (!application_config_system_config_get(&app->app_config, "zone_system", &generic_sys_config)) {
            KERROR("No configuration exists in app config for the zone system. This configuration is required.");
            return false;
        }

        // Parse zone system config from app config.
        zone_system_config zone_sys_config = {0};
        if (!zone_system_deserialize_config(generic_sys_config.configuration_str, &zone_sys_config)) {
            KERROR("Failed to deserialize zone system config, which is required.");
            return false;
        }

        if (!zone_system_intialize(&state->zone_state, &zone_sys_config)) {
            KERROR("Failed to initialize zone system.");
            return false;
        }
    }
    ...
    if (!sui_label_control_create(sui_state, "soi_mono_test_text_black", FONT_TYPE_BITMAP, kname_create("Ubuntu Mono 21px"), 21, "test text 123,\n\tyo!", &state->debug_text_shadow)) {
    ...
    if (!sui_label_control_create(sui_state, "soi_mono_test_text", FONT_TYPE_BITMAP, kname_create("Ubuntu Mono 21px"), 21, "test text 123,\n\tyo!", &state->debug_text)) {
        KERROR("Failed to load basic ui bitmap text.");
        return false;
    } else {
...
b8 application_update(struct application* app, struct frame_data* p_frame_data) {
    game_frame_data* app_frame_data = (game_frame_data*)p_frame_data->application_frame_data;
    if (!app_frame_data) {
        return true;
    }

    game_state* game = (game_state*)app->state;
    if (!game->running) {
        return true;
    }

    kclock_start(&game->update_clock);

    // Update the debug text with camera position.
    vec3 pos = camera_position_get(game->current_camera);
    vec3 rot = camera_rotation_euler_get(game->current_camera);

    viewport* view_viewport = &game->world_viewport;

    f32 near_clip = view_viewport->near_clip;
    f32 far_clip = view_viewport->far_clip;

    zone* current_zone = game->zone_state.current_zone;
    scene* current_scene = current_zone ? &current_zone->zone_scene : 0;
    if (current_scene) {
        if (current_scene->state == SCENE_STATE_LOADED) {
            if (!scene_update(current_scene, p_frame_data)) {
                KWARN("Failed to update main scene.");
            }

            // Update LODs for the scene based on distance from the camera.
            scene_update_lod_from_view_position(current_scene, p_frame_data, pos, near_clip, far_clip);

            editor_gizmo_update(&game->gizmo);

            // HACK: player movement
            if (khandle_is_valid(game->player_xform)) {

                mat4 player_xform = xform_local_get(game->player_xform);
                vec3 player_position = mat4_position(player_xform);
                vec3 forward = mat4_forward(player_xform);
                vec3 right = mat4_right(player_xform);
                ...
                if (game->mode == GAME_MODE_WORLD) {
                    // Move the player
                    if (input_is_key_down(KEY_W)) {
                        velocity = vec3_add(velocity, vec3_mul_scalar(forward, delta * movement_speed));
                    }
                    if (input_is_key_down(KEY_S)) {
                        velocity = vec3_add(velocity, vec3_mul_scalar(forward, delta * -movement_speed));
                    }
                    if (input_is_key_down(KEY_Q)) {
                        velocity = vec3_add(velocity, vec3_mul_scalar(right, delta * -movement_speed));
                    }
                    if (input_is_key_down(KEY_E)) {
                        velocity = vec3_add(velocity, vec3_mul_scalar(right, delta * movement_speed));
                    }
                    if (input_is_key_down(KEY_A)) {
                        quat rotation = quat_from_axis_angle((vec3){0, 1, 0}, -turn_speed * delta, false);
                        xform_rotate(game->player_xform, rotation);
                    }
                ...
                // Apply velocity.
                player_position = vec3_add(player_position, velocity);
                xform_position_set(game->player_xform, player_position);

                // Recalculate local matrices and update the model.
                xform_calculate_local(game->player_xform);
                xform_calculate_local(game->player_mesh_xform);
                player_xform = xform_local_get(game->player_xform);

                // Update follow camera to follow.
                f32 chase_distance = 30.0;
                vec3 backward_offset = vec3_mul_scalar(vec3_forward(), -chase_distance);
                vec3 upward_offset = vec3_create(0.0f, 30.0f, 0.0f);
                vec3 camera_position = vec3_add(vec3_add(player_position, backward_offset), upward_offset);
                camera_position_set(game->vehicle_camera, camera_position);

                // Direction from point to focus object
                vec3 to_obj = vec3_sub(player_position, camera_position);
                ...
                camera_rotation_euler_set_radians(game->vehicle_camera, euler);
            }

        } else if (current_scene->state == SCENE_STATE_UNLOADING) {
            // A final update call is required to unload the scene in this state.
            scene_update(current_scene, p_frame_data);
        } else if (current_scene->state == SCENE_STATE_UNLOADED) {
    ...
    if (game->running) {
        ...
        // Convert to NDC
        f32 mouse_x_ndc = range_convert_f32((f32)mouse_x, 0.0f, (f32)game->width, -1.0f, 1.0f);
        f32 mouse_y_ndc = range_convert_f32((f32)mouse_y, 0.0f, (f32)game->height, -1.0f, 1.0f);
        ...
        total_update_seconds += game->last_update_elapsed;
        total_prepare_seconds += game->prepare_clock.elapsed;
        total_render_seconds += game->render_clock.elapsed;
...
VSync: %s Drawn: %-5u (%-5u shadow pass), Mode: %s, Run time: %s",
        ...
        sui_label_text_set(game->sui_state, &game->debug_text, text_buffer);
        sui_label_text_set(game->sui_state, &game->debug_text_shadow, text_buffer);
    ...
    if (game->mode == GAME_MODE_EDITOR) {
        ...
        sound_pos = pos;
        sound_forward = camera_forward(game->current_camera);
        sound_up = camera_up(game->current_camera);
    } else if (game->mode == GAME_MODE_WORLD) {
        // In world mode, the sound follows the player.
        mat4 vehicle_xform = xform_local_get(game->player_xform);
    ...
    kclock_update(&game->update_clock);
    game->last_update_elapsed = game->update_clock.elapsed;
...
b8 application_prepare_frame(struct application* app, struct frame_data* p_frame_data) {
    ...
    kclock_start(&game->prepare_clock);

    zone* current_zone = game->zone_state.current_zone;
    scene* current_scene = current_zone ? &current_zone->zone_scene : 0;
    viewport* current_viewport = &game->world_viewport;

    // HACK: Using the first light in the collection for now.
    // TODO: Support for multiple directional lights with priority sorting.
    directional_light* dir_light = (current_scene && current_scene->dir_lights) ? &current_scene->dir_lights[0] : 0;
    ...
    u32 node_count = game->forward_graph.node_count;
    for (u32 i = 0; i < node_count; ++i) {
        rendergraph_node* node = &game->forward_graph.nodes[i];
        if (strings_equali(node->name, "sui")) {
            ui_rendergraph_node_set_atlas(node, game->sui_state->atlas_texture);
            ...
            if (!standard_ui_system_render(game->sui_state, 0, p_frame_data, &render_data)) {
            ...
            forward_rendergraph_node_viewport_set(node, game->world_viewport);
            ...
            if (current_scene && current_scene->state == SCENE_STATE_LOADED) {
                ...
                u32 skybox_count = darray_length(current_scene->skyboxes);
                forward_rendergraph_node_set_skybox(node, skybox_count ? &current_scene->skyboxes[0] : 0);

                // SCENE
                scene_render_frame_prepare(current_scene, p_frame_data);
                ...
                forward_rendergraph_node_render_mode_set(node, game->render_mode);
                ...
                forward_rendergraph_node_irradiance_texture_set(node, p_frame_data, current_scene->skyboxes ? current_scene->skyboxes[0].cubemap : texture_system_request(kname_create(DEFAULT_CUBE_TEXTURE_NAME), INVALID_KNAME, 0, 0));

                // Camera frustum culling and count
                viewport* v = current_viewport;
                vec3 forward = camera_forward(game->current_camera);
                vec3 target = vec3_add(game->current_camera->position, vec3_mul_scalar(forward, far));
                vec3 up = camera_up(game->current_camera);
                // TODO: move frustum to be managed by camera it is attached to.
                frustum camera_frustum = frustum_create(&game->current_camera->position, &target,
                ...
                if (!scene_water_plane_query(current_scene, &camera_frustum, game->current_camera->position, p_frame_data, &water_plane_count, 0)) {
                ...
                if (!scene_water_plane_query(current_scene, &camera_frustum, game->current_camera->position, p_frame_data, &water_plane_count, &planes)) {
                    KERROR("Failed to query scene for water planes.");
                }
            ...
            if (dir_light) {
                ...
                mat4 shadow_dist_projection = mat4_perspective(
                    ...
                    near,
                    far);
                mat4 cam_view_proj = mat4_transposed(mat4_mul(camera_view_get(game->current_camera), shadow_dist_projection));
            ...
            debug_rendergraph_node_viewport_set(node, game->world_viewport);
            debug_rendergraph_node_view_projection_set(
                node,
                camera_view_get(game->current_camera),
                camera_position_get(game->current_camera),
                current_viewport->projection);

            u32 debug_geometry_count = 0;
            geometry_render_data* debug_geometries = 0;
            if (current_scene) {
                if (!scene_debug_render_data_query(current_scene, &debug_geometry_count, 0)) {
                ...
                if (debug_geometry_count) {
                    ...
                    if (!scene_debug_render_data_query(current_scene, &debug_geometry_count, &debug_geometries)) {
            ...
            editor_gizmo_rendergraph_node_viewport_set(node, game->world_viewport);
            editor_gizmo_rendergraph_node_view_projection_set(
                node,
                camera_view_get(game->current_camera),
                camera_position_get(game->current_camera),
                current_viewport->projection);
            if (!editor_gizmo_rendergraph_node_gizmo_set(node, &game->gizmo)) {
                // NOTE: Not going to abort the whole graph for this failure, but will bleat about it loudly.
    ...
    kclock_update(&game->prepare_clock);
    return true;
}
...
b8 application_render_frame(struct application* app, struct frame_data* p_frame_data) {
    ...
    kclock_start(&game->render_clock);

    // Execute the rendergraph.
    if (!rendergraph_execute_frame(&game->forward_graph, p_frame_data)) {
        KERROR("Rendergraph failed to execute frame, see logs for details.");
        return false;
    }

    kclock_update(&game->render_clock);
...
void application_on_window_resize(struct application* app, const struct kwindow* window) {
    ...
    rect_2d world_vp_rect = vec4_create(0.0f, 0.0f, game->width, game->height); // vec4_create(half_width + 20.0f, 20.0f, half_width - 40.0f, state->height - 40.0f);
    viewport_resize(&game->world_viewport, world_vp_rect);

    // UI Viewport
    rect_2d ui_vp_rect = vec4_create(0.0f, 0.0f, game->width, game->height);
    viewport_resize(&game->ui_viewport, ui_vp_rect);

    // Move debug text to new bottom of screen.
    sui_control_position_set(game->sui_state, &game->debug_text, vec3_create(20, game->height - 95, 0));
    sui_control_position_set(game->sui_state, &game->debug_text_shadow, vec3_create(21, game->height - 94, 0));
}
...
void application_shutdown(struct application* app) {
    ...
    // The zone system will auto-unload any currently loaded zone.
    zone_system_shutdown(&game->zone_state);

    // Also destroy the rendergraph.
    rendergraph_destroy(&game->forward_graph);

#ifdef KOHI_DEBUG
    debug_console_unload(&game->debug_console);
#endif
}
...
static void setup_keymaps(application* app) {
    ...
    // Global keymap
    game->global_keymap = keymap_create();
    keymap_binding_add(&game->global_keymap, KEY_ESCAPE, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_escape_callback);
    keymap_binding_add(&game->global_keymap, KEY_V, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_debug_vsync_toggle);
    keymap_binding_add(&game->global_keymap, KEY_GRAVE, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_change_visibility);
    keymap_binding_add(&game->global_keymap, KEY_L, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_load_scene);
    keymap_binding_add(&game->global_keymap, KEY_U, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_unload_scene);

    // World mode keymap
    game->world_keymap = keymap_create();
    keymap_binding_add(&game->world_keymap, KEY_C, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, change_current_camera);

    // Editor mode keymap
    game->editor_keymap = keymap_create();
    /* state->editor_keymap.overrides_all = true; */
    keymap_binding_add(&game->editor_keymap, KEY_C, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, change_current_camera);

    keymap_binding_add(&game->editor_keymap, KEY_A, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_yaw);
    keymap_binding_add(&game->editor_keymap, KEY_LEFT, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_yaw);

    keymap_binding_add(&game->editor_keymap, KEY_D, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_yaw);
    keymap_binding_add(&game->editor_keymap, KEY_RIGHT, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_yaw);

    keymap_binding_add(&game->editor_keymap, KEY_UP, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_pitch);
    keymap_binding_add(&game->editor_keymap, KEY_DOWN, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_pitch);

    keymap_binding_add(&game->editor_keymap, KEY_W, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_move_forward);
    keymap_binding_add(&game->editor_keymap, KEY_S, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_move_backward);
    keymap_binding_add(&game->editor_keymap, KEY_Q, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_move_left);
    keymap_binding_add(&game->editor_keymap, KEY_E, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_move_right);
    keymap_binding_add(&game->editor_keymap, KEY_SPACE, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_move_up);
    keymap_binding_add(&game->editor_keymap, KEY_X, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_move_down);

    keymap_binding_add(&game->editor_keymap, KEY_0, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_CONTROL_BIT, app, game_on_set_render_mode_default);
    keymap_binding_add(&game->editor_keymap, KEY_1, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_CONTROL_BIT, app, game_on_set_render_mode_lighting);
    keymap_binding_add(&game->editor_keymap, KEY_2, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_CONTROL_BIT, app, game_on_set_render_mode_normals);
    keymap_binding_add(&game->editor_keymap, KEY_3, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_CONTROL_BIT, app, game_on_set_render_mode_cascades);
    keymap_binding_add(&game->editor_keymap, KEY_4, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_CONTROL_BIT, app, game_on_set_render_mode_wireframe);

    keymap_binding_add(&game->editor_keymap, KEY_1, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_set_gizmo_mode);
    keymap_binding_add(&game->editor_keymap, KEY_2, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_set_gizmo_mode);
    keymap_binding_add(&game->editor_keymap, KEY_3, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_set_gizmo_mode);
    keymap_binding_add(&game->editor_keymap, KEY_4, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_set_gizmo_mode);
    keymap_binding_add(&game->editor_keymap, KEY_G, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_gizmo_orientation_set);

    // ctrl s
    keymap_binding_add(&game->editor_keymap, KEY_S, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_CONTROL_BIT, app, game_on_save_scene);

    // A console-specific keymap. Is not pushed by default.
    game->console_keymap = keymap_create();
    game->console_keymap.overrides_all = true;
    keymap_binding_add(&game->console_keymap, KEY_GRAVE, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_change_visibility);
    keymap_binding_add(&game->console_keymap, KEY_ESCAPE, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_change_visibility);

    keymap_binding_add(&game->console_keymap, KEY_PAGEUP, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_scroll);
    keymap_binding_add(&game->console_keymap, KEY_PAGEDOWN, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_scroll);
    keymap_binding_add(&game->console_keymap, KEY_PAGEUP, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_scroll_hold);
    keymap_binding_add(&game->console_keymap, KEY_PAGEDOWN, KEYMAP_BIND_TYPE_HOLD, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_scroll_hold);

    keymap_binding_add(&game->console_keymap, KEY_UP, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_history_back);
    keymap_binding_add(&game->console_keymap, KEY_DOWN, KEYMAP_BIND_TYPE_PRESS, KEYMAP_MODIFIER_NONE_BIT, app, game_on_console_history_forward);

// If this was done with the console open, push its keymap.
#if KOHI_DEBUG
    b8 console_visible = debug_console_visible(&game->debug_console);
    if (console_visible) {
        input_keymap_push(&game->console_keymap);
    }
#endif
}
...
static void change_current_camera(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    if (game->mode == GAME_MODE_WORLD) {
        ...
        game->current_camera = game->editor_camera;
        KTRACE("Editor camera: %f %f %f", game->current_camera->position.x, game->current_camera->euler_rotation.y, game->current_camera->euler_rotation.z);
        if (!input_keymap_pop()) {
            KERROR("No keymap was popped during world->editor");
        }
        input_keymap_push(&game->editor_keymap);
    } else if (game->mode == GAME_MODE_EDITOR) {
        game->mode = GAME_MODE_WORLD;
        game->current_camera = game->vehicle_camera;
        KTRACE("Vehicle camera: %f %f %f", game->current_camera->position.x, game->current_camera->euler_rotation.y, game->current_camera->euler_rotation.z);
        if (!input_keymap_pop()) {
...
static void game_on_yaw(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_yaw(game->editor_camera, f * get_engine_delta_time());
}
...
static void game_on_pitch(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_pitch(game->editor_camera, f * get_engine_delta_time());
}
...
static void game_on_move_forward(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_move_forward(game->editor_camera, game->editor_camera_forward_move_speed * get_engine_delta_time());
}
...
static void game_on_move_backward(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_move_backward(game->editor_camera, game->editor_camera_backward_move_speed * get_engine_delta_time());
}
...
static void game_on_move_left(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_move_left(game->editor_camera, game->editor_camera_forward_move_speed * get_engine_delta_time());
}
...
static void game_on_move_right(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_move_right(game->editor_camera, game->editor_camera_forward_move_speed * get_engine_delta_time());
}
...
static void game_on_move_up(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_move_up(game->editor_camera, game->editor_camera_forward_move_speed * get_engine_delta_time());
}
...
static void game_on_move_down(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    camera_move_down(game->editor_camera, game->editor_camera_forward_move_speed * get_engine_delta_time());
}
...
static void game_on_console_change_visibility(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    debug_console_visible_set(&game->debug_console, console_visible);
    if (console_visible) {
        input_keymap_push(&game->console_keymap);
    } else {
        input_keymap_pop();
...
static void game_on_set_render_mode_default(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    game->render_mode = RENDERER_VIEW_MODE_DEFAULT;
}

static void game_on_set_render_mode_lighting(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    game->render_mode = RENDERER_VIEW_MODE_LIGHTING;
}

static void game_on_set_render_mode_normals(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    game->render_mode = RENDERER_VIEW_MODE_NORMALS;
}

static void game_on_set_render_mode_cascades(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    game->render_mode = RENDERER_VIEW_MODE_CASCADES;
}

static void game_on_set_render_mode_wireframe(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    game->render_mode = RENDERER_VIEW_MODE_WIREFRAME;
}
...
static void game_on_set_gizmo_mode(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    editor_gizmo_mode_set(&game->gizmo, mode);
}
...
static void game_on_gizmo_orientation_set(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    editor_gizmo_orientation orientation = editor_gizmo_orientation_get(&game->gizmo);
    orientation++;
    if (orientation > EDITOR_GIZMO_ORIENTATION_MAX) {
        orientation = 0;
    }
    editor_gizmo_orientation_set(&game->gizmo, orientation);
}

static void game_on_load_scene(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {

    // HACK: Trigger loading of a specifc scene.
    // HACK: Also choose a specific spawn point.
    static u8 spawn_point_id = 0;
    spawn_point_id++;
    spawn_point_id %= 2;

    // Just execute it as a console command as if it were entered in the debug console.
    char* command = string_format("go_to_zone %s %u", "test_zone", spawn_point_id);
...
static void game_on_save_scene(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    if (game->zone_state.current_zone) {
        if (game->zone_state.current_zone->zone_scene.state == SCENE_STATE_LOADED) {
            KDEBUG("Saving zone scene...");
            if (!scene_save(&game->zone_state.current_zone->zone_scene)) {
                KERROR("Error saving zone scene");
            }
        } else {
            KERROR("Current scene is not in a loaded state, and cannot be saved.");
        }
    } else {
        KERROR("No current scene to save.");
    }
}
...
// No-op unless a debug build.
    ...
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    debug_console_history_back(&game->debug_console);
#endif
}
...
static void game_on_console_history_forward(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    ...
    application* app = (application*)user_data;
    game_state* game = (game_state*)app->state;
    debug_console_history_forward(&game->debug_console);
#endif
}
static void game_on_debug_vsync_toggle(keys key, keymap_entry_bind_type type, keymap_modifier modifiers, void* user_data) {
    char cmd[30];
...
static b8 game_register_events(application* app) {
    if (!event_register(GAME_EVENT_CODE_ZONE_LOADED, app, game_on_event)) {
        KERROR("%s - Failed to register event. See logs for details.", __FUNCTION__);
        return false;
    }
...
static void game_unregister_events(application* app) {
    if (!event_unregister(GAME_EVENT_CODE_ZONE_LOADED, app, game_on_event)) {
        KERROR("%s - Failed to unregister event. See logs for details.", __FUNCTION__);
    }
}

static b8 game_register_commands(application* app) {
    if (!console_command_register("exit", 0, app, game_command_exit)) {
        KERROR("%s - Failed to register console command. See logs for details.", __FUNCTION__);
        return false;
    }
    if (!console_command_register("quit", 0, app, game_command_exit)) {
        KERROR("%s - Failed to register console command. See logs for details.", __FUNCTION__);
        return false;
    }

    if (!console_command_register("load_zone", 2, app, game_command_load_zone)) {
        KERROR("%s - Failed to register console command. See logs for details.", __FUNCTION__);
        return false;
    }
    if (!console_command_register("go_to_zone", 2, app, game_command_go_to_zone)) {
        KERROR("%s - Failed to register console command. See logs for details.", __FUNCTION__);
        return false;
    }
    if (!console_command_register("unload_zone", 0, app, game_command_unload_zone)) {
        KERROR("%s - Failed to register console command. See logs for details.", __FUNCTION__);
        return false;
    }
    if (!console_command_register("save_zone", 0, app, game_command_save_zone)) {
        KERROR("%s - Failed to register console command. See logs for details.", __FUNCTION__);
        return false;
    }
...
static void game_unregister_commands(application* app) {
    if (!console_command_unregister("exit")) {
        KERROR("%s - Failed to unregister console command. See logs for details.", __FUNCTION__);
    }
    if (!console_command_unregister("quit")) {
        KERROR("%s - Failed to unregister console command. See logs for details.", __FUNCTION__);
    }
    if (!console_command_unregister("load_zone")) {
        KERROR("%s - Failed to unregister console command. See logs for details.", __FUNCTION__);
    }
    if (!console_command_unregister("unload_zone")) {
        KERROR("%s - Failed to unregister console command. See logs for details.", __FUNCTION__);
    }
    if (!console_command_unregister("go_to_zone")) {
        KERROR("%s - Failed to unregister console command. See logs for details.", __FUNCTION__);
    }
    if (!console_command_unregister("save_zone")) {
        KERROR("%s - Failed to unregister console command. See logs for details.", __FUNCTION__);
    }
}
...
static b8 game_on_event(u16 code, void* sender, void* listener_inst, event_context context) {
    ...
    switch (code) {
    case GAME_EVENT_CODE_ZONE_LOADED: {
        // A new zone has been loaded successfully.
        zone* z = (zone*)sender;
        scene* current_scene = &z->zone_scene;

        u8 spawn_point_id = context.data.u8[0];

        vec3 spawn_position;
        quat spawn_rotation;
        b8 result = zone_system_current_zone_spawn_get(&game->zone_state, spawn_point_id, &spawn_position, &spawn_rotation);
        KASSERT_MSG(result, "zone_system_current_zone_spawn_get returned false - did you forget a pointer?");

        KDEBUG(
            "Spawning player at position: [x=%.2f, y=%.2f, z=%.2f] and setting rotation: [x=%.2f, y=%.2f, z=%.2f, w=%.2f]",
            spawn_position.x,
            spawn_position.y,
            spawn_position.z,
            spawn_rotation.x,
        ...
        // HACK: Get xform of player once added. This is in the scene file now, but should be loaded after the fact
        // in code instead.
        result = scene_node_xform_get_by_name(current_scene, kname_create("player"), &game->player_xform);
        KASSERT_MSG(result, "Unable to get player node xform.");

        result = scene_node_xform_get_by_name(current_scene, kname_create("player_mesh"), &game->player_mesh_xform);
        KASSERT_MSG(result, "Unable to get player mesh.");

        //  Move the player to the spawn point at the appropriate index.
        xform_position_set(game->player_xform, spawn_position);
        // Also set rotation.
        xform_rotation_set(game->player_xform, spawn_rotation);
        // Ensure it is recalculated. FIXME: This shouldn't be required at this level.
        xform_calculate_local(game->player_xform);
...
static void trigger_zone_load(console_command_context context) {
    application* app = (application*)context.listener;
    game_state* game = (game_state*)app->state;

    // Ensure there are 2 arguments. The first should be the zone name and the second should be the spawn_point_id.
    if (context.argument_count != 2) {
        KERROR("%s - Expected 2 arguments but got %u", context.argument_count);
        return;
    }

    kname zone_name = kname_create(context.arguments[0].value);
    u8 spawn_point_id = INVALID_ID_U8;
    if (!string_to_u8(context.arguments[1].value, &spawn_point_id)) {
        KERROR("load_zone - Failed to convert argument 1 to u8.");
        return;
    }

    // Trigger loading of the scene.
    if (!zone_system_load(&game->zone_state, zone_name, spawn_point_id)) {
        KERROR("Failed to load zone! See logs for details.");
    }
}

static void trigger_zone_unload(console_command_context context) {
    application* app = (application*)context.listener;
    game_state* game = (game_state*)app->state;

    // Trigger unloading of the scene.
    zone_system_unload_current(&game->zone_state, true);
}
...
static void game_command_load_zone(console_command_context context) {
    trigger_zone_load(context);
}

static void game_command_unload_zone(console_command_context context) {
    trigger_zone_unload(context);
}
...
static void game_command_save_zone(console_command_context context) {
    ...
    // TODO: This should be routed through the zone system.
    if (game->zone_state.current_zone) {
        if (game->zone_state.current_zone->zone_scene.state == SCENE_STATE_LOADED) {
            KDEBUG("Saving zone scene...");
            if (!scene_save(&game->zone_state.current_zone->zone_scene)) {

...
b8 vulkan_device_create(vulkan_context* context, b8 require_discrete_gpu) {
    ...
    VK_CHECK(rhi->kvkCreateCommandPool(
        context->device.logical_device,
        &pool_create_info,
        context->allocator,
...
void vulkan_device_query_swapchain_support(
    ...
    // Surface formats
    VK_CHECK(rhi->kvkGetPhysicalDeviceSurfaceFormatsKHR(
        physical_device,
        surface,
    ...
    // Present modes
    VK_CHECK(rhi->kvkGetPhysicalDeviceSurfacePresentModesKHR(
        physical_device,
        surface,
